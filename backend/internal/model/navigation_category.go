...
type NavigationCategory struct {
	BaseModel
	ParentID    *uint                `gorm:"index:idx_nav_cat_parent_id;comment:父分类ID" json:"parent_id"`
	...
	Sort        int                  `gorm:"type:int;default:0;index:idx_nav_cat_sort;comment:排序" json:"sort"`
