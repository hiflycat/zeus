...
func (s *FormTemplateService) Update(id uint, t *model.FormTemplate) error {
	return global.GetDB().Model(&model.FormTemplate{}).Where("id = ?", id).Updates(map[string]interface{}{
		"name":        t.Name,
...
func (s *FormTemplateService) Delete(id uint) error {
	// 检查是否有工单类型正在使用该模板
	var count int64
	global.GetDB().Model(&model.TicketType{}).Where("template_id = ?", id).Count(&count)
	if count > 0 {
		return errors.New("该模板正在被工单类型使用，无法删除")
	}

	// 删除关联的字段
	if err := global.GetDB().Where("template_id = ?", id).Delete(&model.FormField{}).Error; err != nil {
		return err
	}
	return global.GetDB().Delete(&model.FormTemplate{}, id).Error
}

func (s *FormTemplateService) GetByID(id uint) (*model.FormTemplate, error) {
	var t model.FormTemplate
	if err := global.GetDB().Preload("Fields", func(db *gorm.DB) *gorm.DB {
...
func (s *FormTemplateService) List(page, pageSize int, keyword string) ([]model.FormTemplate, int64, error) {
	...
	var total int64
	db := global.GetDB().Model(&model.FormTemplate{})
...
func (s *FormTemplateService) ListEnabled() ([]model.FormTemplate, error) {
	var templates []model.FormTemplate
	if err := global.GetDB().Where("enabled = ?", true).Order("name ASC").Find(&templates).Error; err != nil {
		return nil, err
	}
...
func (s *FormTemplateService) GetFields(templateID uint) ([]model.FormField, error) {
	...
	if err := global.GetDB().Where("template_id = ?", templateID).Order("sort_order ASC").Find(&fields).Error; err != nil {
		return nil, err
	}
	return fields, nil
}
...
func (s *FormTemplateService) CreateField(field *model.FormField) error {
	// 获取当前最大排序
	var maxSort int
	global.GetDB().Model(&model.FormField{}).Where("template_id = ?", field.TemplateID).
		Select("COALESCE(MAX(sort_order), 0)").Scan(&maxSort)
	field.SortOrder = maxSort + 1
	return global.GetDB().Create(field).Error
}

// UpdateField 更新单个字段
func (s *FormTemplateService) UpdateField(id uint, field *model.FormField) error {
	return global.GetDB().Model(&model.FormField{}).Where("id = ?", id).Updates(field).Error
}
...
func (s *FormTemplateService) DeleteField(id uint) error {
	// 检查是否有工单数据正在使用该字段
	var count int64
	global.GetDB().Model(&model.TicketData{}).Where("field_id = ?", id).Count(&count)
	if count > 0 {
		return errors.New("该字段已有工单数据，无法删除")
	}
	return global.GetDB().Delete(&model.FormField{}, id).Error
}
