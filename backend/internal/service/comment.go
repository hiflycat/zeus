...
func (s *CommentService) Create(comment *model.TicketComment) error {
	return migrations.GetDB().Create(comment).Error
}

// Delete 删除评论
func (s *CommentService) Delete(id, userID uint) error {
	// 只能删除自己的评论
	return migrations.GetDB().Where("id = ? AND user_id = ?", id, userID).Delete(&model.TicketComment{}).Error
}
...
func (s *CommentService) GetByTicketID(ticketID uint) ([]model.TicketComment, error) {
	...
	if err := migrations.GetDB().Preload("User").
